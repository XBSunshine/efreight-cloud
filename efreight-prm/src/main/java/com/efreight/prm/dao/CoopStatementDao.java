...
public interface CoopStatementDao {
    ...
    @Select("CALL prm_P_report_receivable_age_EF(#{query.orgId}, #{query.overdue}, #{query.excess}, #{query.coopName}, #{query.customerResponsible}," +
    ...
    @Select("select  " +
            ...
            " CASE IFNULL(PCAS.bill_template,PCS.bill_template)\n" +
            "            WHEN 'BJS' THEN '华北'\n" +
            "            WHEN 'CAN' THEN '华南'\n" +
            "            WHEN 'SHA' THEN '华东 '\n" +
            "            WHEN 'XIY' THEN '西北'\n" +
    ...
    List<CoopStatementDetail> listDetailCoopStatement(@Param("orgId") Integer orgId, @Param("coopId") Integer coopId);
