...
public interface OrgCouldUserMapper extends BaseMapper<OrgCouldUser>{
	...
	@Select({"<script>",
		  ...
		  " WHERE t.org_type !=0  ",
		     ...
		     "  AND (((IFNULL(a.af_order_num,0)+IFNULL(s.sc_order_num,0))<![CDATA[>]]>0  or IFNULL(p.pc_coop_num,0) <![CDATA[>]]> 2 or t.is_Intention_user = 1 or subscription.subscription_num > 0) and (t.is_Intention_user = 1 or t.is_Intention_user = -1 or t.is_Intention_user is null) and p2.org_name NOT IN ('标准版','专业版','创业版','旗舰版') and p2.org_name not like CONCAT('%', '内部', '%')) ",
		     "</when>",
