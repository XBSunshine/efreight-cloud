...
public interface StatementMapper extends BaseMapper<Statement> {
    ...
    @Select({"<script>",
            "    SELECT a.*,p.social_credit_code,p.coop_address,p.phone_number,p.bank_number,p.bank_name, ",
//        ",b.Awb_Number,b.Order_Code,b.Customer_Number ",
            "IFNULL(m.apply_remark,a.invoice_remark) AS apply_remark,",
            " m.invoice_status,m.creator_name AS invoice_creator_name,m.create_time AS invoice_create_time ",
            ...
            "INNER JOIN ",
            " (select  statement_id  from css_income_invoice_detail where 1=1 ",
            "<when test='bean.invoiceNum!=null and bean.invoiceNum!=\"\"'>",
            " AND invoice_num like \"%\"#{bean.invoiceNum}\"%\" ",
            "</when>",
            "<when test='bean.invoiceTitle!=null and bean.invoiceTitle!=\"\"'>",
            " AND invoice_title like \"%\"#{bean.invoiceTitle}\"%\" ",
            "</when>",
            "<when test='bean.invoiceDateStart!=null and bean.invoiceDateStart!=\"\"'>",
            " AND invoice_date  <![CDATA[ >= ]]> #{bean.invoiceDateStart} ",
            "</when>",
            "<when test='bean.invoiceDateEnd!=null and bean.invoiceDateEnd!=\"\"'>",
            " AND invoice_date  <![CDATA[ <= ]]> #{bean.invoiceDateEnd} ",
            "</when>",
            " group by statement_id ",
            ") invoice on invoice.statement_id = a.statement_id ",
            "</when>",
            
            // css_income_invoice
            " LEFT JOIN css_income_invoice m ON a.statement_id = m.statement_id ",
            ...
            " AND a.writeoff_complete !=1 and m.invoice_status is not null and m.invoice_status!=-1 ",
            "</when>",
            "<when test='bean.statementStatus==\"-1\"'>",
            " AND a.writeoff_complete IS NULL and m.statement_id IS NULL",
            "</when>",
            "<when test='bean.statementStatus==\"1\"'>",
            ...
            "</when>",
            "<when test='bean.statementStatus==\"待开票\"'>",
            " AND m.invoice_status = -1  and m.statement_id IS not NULL",
            "</when>",
            "<when test='bean.statementStatus==\"部分开票\"'>",
            " AND m.invoice_status = 0  and m.statement_id IS not NULL and a.writeoff_complete IS NULL",
            "</when>",
            "<when test='bean.statementStatus==\"开票完毕\"'>",
            " AND m.invoice_status = 1  and m.statement_id IS not NULL and a.writeoff_complete IS NULL",
            "</when>",
            ...
            " AND m.creator_name like  \"%\"#{bean.invoiceCreatorName}\"%\" ",
            "</when>",
            "<when test='bean.invoiceCreateTimeStart!=null and bean.invoiceCreateTimeStart!=\"\"'>",
            " AND m.create_time  <![CDATA[ >= ]]> #{bean.invoiceCreateTimeStart} ",
            "</when>",
            "<when test='bean.invoiceCreateTimeEnd!=null and bean.invoiceCreateTimeEnd!=\"\"'>",
            " AND m.create_time  <![CDATA[ <= ]]> #{bean.invoiceCreateTimeEnd}",
            "</when>",
...
//       ",b.Awb_Number,b.Order_Code,b.Customer_Number ",
           ...
           " LEFT JOIN css_income_invoice m ON a.statement_id = m.statement_id ",
            ...
            " WHERE 1=1 and a.org_id=#{bean.orgId} ",
            "<when test='bean.statementStatus==\"-2\"'>",
            " AND (a.writeoff_complete =0 or m.invoice_status = 1  ) and (a.writeoff_complete is null or a.writeoff_complete != 1)",
            "</when>",
            "<when test='bean.statementStatus==\"-1\"'>",
            " AND a.writeoff_complete IS NULL and m.statement_id IS NULL",
            "</when>",
    ...
    @Update("update css_debit_note set\n"
            + " statement_id=#{statement_id},org_bank_config_id=#{orgBankConfigId},row_uuid=#{row_uuid} "
            + " where   org_id=#{org_id} and debit_note_id in (${ids}) ")
    void updateDebitNote(@Param("org_id") Integer org_id, @Param("ids") String ids, @Param("statement_id") Integer statement_id,@Param("orgBankConfigId") Integer orgBankConfigId, @Param("row_uuid") String row_uuid);
